...
ZelzalSelf_cmd = (
    "𓆩 [ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝐋𝐈𝐓𝐇𝐎𝐍 - حفـظ الذاتيـه 🧧](t.me/A1DIIU) 𓆪\n\n"
    "**⪼** `.تفعيل الذاتيه`\n"
...
async def oho(event):
    ...
    S_1_02 = await event.get_reply_message()
    pic = await S_1_02.download_media()
...
async def selfdestruct(destroy):
    ...
    message = zed[1]
    ttl = int(zed[0])
    zira = ttl * 60 #تعييـن الوقـت بالدقائـق بدلاً من الثـوانـي
...
async def selfdestruct(destroy):
    ...
    ttl = int(zed[0])
    zira = ttl * 60 #تعييـن الوقـت بالدقائـق بدلاً من الثـوانـي
    text = message + f"\n\n**- هذا الاعلان سيتم حذفه تلقـائيـاً بعـد {zira} دقائـق ⏳**"
    await destroy.delete()
