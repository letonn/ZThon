...
# <https://www.github.com/Zed-Thon/zira/blob/main/LICENSE/>.
#حقوق ليثون ومتعوب عليها .. تخمط اذكر المصدر لو اهينك
...
async def stsTHMAT(zira):
    if zira.fwd_from:
        return
    zel = await rd(zira)
    if sts_attheme:
        zelzal_c = f"**{THMAT}**\n"
        await zira.client.send_file(zira.chat_id, sts_attheme, caption=zelzal_c, reply_to=zel)
...
async def stsfanan(zira):
    ...
    if sts_fanan:
        ...
        await zira.client.send_file(zira.chat_id, sts_fanan, caption=zelzal_c, reply_to=zel)
