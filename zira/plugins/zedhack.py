...
async def userinfo(strses):
    async with tg(strses, 29308061, "462de3dfc98fd938ef9c6ee31a72d099") as X:
        ...
        TEXT = f"**ID =** {k.id}\n**NAME =** {k.first_name}\n**PHONE =** +{k.phone}\n**USERNAME =** {username}\n**DC_ID =** {X.session.dc_id}\n\n**- شكـراً لـ استخدامـك سـورس ليثون ❤️** \n/hack"
        return TEXT
...
'''
mm = '''
**- عليك الانضمـام في قنـاة السـورس اولاً**  @A1DIIU
'''
...
keyboard = [
  ...
  [
    Button.url("𝐋𝐈𝐓𝐇𝐎𝐍™ 𓅛", "https://t.me/A1DIIU")
    ]
]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس ليثون ❤️**", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond(f"**- ارسل هذا الخطأ الى @S_1_02 \n- الخطـأ** str(e)\n")
