...
async def cpu(event):
    ...
    await edit_or_reply(
        event, f"**[letonn/LltHoN1](tg://need_update_for_some_feature/) CPU Model:**\n{o}"
    )
...
async def _(event):
    ...
    sandy = await edit_or_reply(
        ...
        f"**•⎆┊يتـم الان اعـادة تشغيـل بـوت ليثون**\n"
...
async def _(event):
    cmd = "ls zira/plugins"
    o = (await _zedutils.runcmd(cmd))[0]
    OUTPUT = f"**⌔∮ [𝗦𝗢𝗨𝗥𝗖𝗘 𝐋𝐈𝐓𝐇𝐎𝐍](tg://need_update_for_some_feature/) الالاضافات:**\n{o}"
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = (
        f"**[𝗦𝗢𝗨𝗥𝗖𝗘 𝐋𝐈𝐓𝐇𝐎𝐍](tg://need_update_for_some_feature/) - فـارات تنصيبـك هـي:**\n\n\n{o}"
    )
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝐋𝐈𝐓𝐇𝐎𝐍](tg://need_update_for_some_feature/) - سرعـة السيرفـر**\n**- تم حسـاب سرعـة سيرفـر البـوت الخـاص بك :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
async def zeddd(event): # Code By T.me/S_1_02
    uname = platform.uname()
    zedt = "**- تاريخ تنصيبـك لـ بـوت ليثون - 𓆩𝙎𝙊𝙐𝙍𝘾𝞝 𝐋𝐈𝐓𝐇𝐎𝐍𓆪**\n\n"
